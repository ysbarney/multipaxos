...
public:
  ...
  virtual int MasterPropose(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID,
			    SMCtx * poSMCtx) = 0;

  virtual int ReportStatus(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID,
			   SMCtx * poSMCtx) = 0;
  ...
  virtual int SetMinChosenInstanceID(const int iGroupIdx, const uint64_t& llInstanceID) = 0;
  ...
  //To avoid threads be holded too long time, we use this threshold to reject some propose to
  //control thread's wait time.
...
protected:
  ...
  virtual int OnReceiveMessage(const unsigned short iport, const char * pcMessage, const int iMessageLen) = 0;
