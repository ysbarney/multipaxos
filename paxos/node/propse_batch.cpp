...
#include <algorithm>
 ...
 int ProposeBatch :: Propose(const std::string & sValue, uint64_t & llInstanceID, uint32_t & iBatchIndex, SMCtx * poSMCtx)
 {
	 ...
	 if (ret != 0)
	 {
		 PAXOSLOG_ERROR << "GetNotifier fail, ret " << ret;
		 ...
		 SMCtx * poSMCtx, Notifier * poNotifier)
 {
	 ...
	 if (NeedBatch())
	 {
		 PAXOSLOG_DEBUG << "direct batch, queue size " << m_oQueue.size() << " value size " << m_iNowQueueValueSize;
 ...
 void ProposeBatch :: PluckProposal(std::vector<PendingProposal> & vecRequest)
 {
	 ...
	 {
		 PAXOSLOG_DEBUG << "pluck " << vecRequest.size() << " request";
	 }
 }
 ...
 void ProposeBatch :: DoPropose(std::vector<PendingProposal> & vecRequest)
 {
	 ...
	 if (bSucc)
	 {
		 ...
		 if (ret != 0)
		 {
			 PAXOSLOG_ERROR << "real propose fail, ret " << ret;
		 }
	 }
	 else
	 {
		 PAXOSLOG_ERROR << "BatchValues SerializeToString fail";
