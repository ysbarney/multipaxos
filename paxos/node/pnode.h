...
public:
  ...
  int Init(const Options & oOptions, NetWork *& poNetWork, NetWork *&poExtraNetWork);
...
public:
  ...
  int MasterPropose(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID, SMCtx * poSMCtx);
  int ReportStatus(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID, SMCtx * poSMCtx);
  int Forward(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID, SMCtx * poSMCtx);
...
public:
  ...
  int OnRecvPaxosMessage(const char * pcMessage, const int iMessageLen);
  int OnRecvExtraMessage(const char * pcMessage, const int iMessageLen);
...
public:
  ...
  const NodeInfo GetMasterExtra(const int iGroupIdx);
  const NodeInfo GetHBMaster(const int iGroupIdx) ;
  const NodeInfo GetMasterWithVersion(const int iGroupIdx, uint64_t & llVersion);
  const bool IsIMMaster(const int iGroupIdx);
  const bool IsIMHBMaster(const int iGroupIdx);
