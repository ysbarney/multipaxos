...
{
public:
  Instance(const Config * poConfig, const LogStorage * poLogStorage, const MsgTransport * poMsgTransport,
...
public:
  ...
  QueueProposer* GetQueueProposer();
  SMFac* GetSMFac();
...
public:
  void OnTimeout(const uint32_t iTimerID, const int iType, const uint64_t llInstanceID);

public:
  void AddStateMachine(StateMachine * poSM);

  bool SMExecute(const uint64_t llInstanceID, const std::string & sValue,
		 SMCtx * poSMCtx);
...
private:
  ...
  int ProtectionLogic_IsCheckpointInstanceIDCorrect(const uint64_t llCPInstanceID,
...
private:
  ...
  QueueProposer m_oQueueProposer;
