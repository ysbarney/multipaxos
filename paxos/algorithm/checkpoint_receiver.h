/*
 * Module_name: checkpoint_receiver.h
 * Author: Barneyliu
 * Time: 2019-01-10
 * Description:
 *
...
public:
  ...
  virtual ~CheckpointReceiver();

  void Reset();
  int NewReceiver(const nodeid_t iSenderNodeID, const uint64_t llUUID);

  const bool IsReceiverFinish(const nodeid_t iSenderNodeID, const uint64_t llUUID,
			      const uint64_t llEndSequence);
