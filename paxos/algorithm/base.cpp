...
int Base :: PackMsg(const PaxosMsg & oPaxosMsg, std::string & sBuffer) {
  ...
  if (!bSucc) {
    PAXOSLOG_ERROR << "PaxosMsg.SerializeToString fail, skip this msg";
    return -1;
  }
...
int Base :: PackCheckpointMsg(const CheckpointMsg & oCheckpointMsg, std::string & sBuffer) {
  ...
  if (!bSucc) {
    PAXOSLOG_ERROR << "CheckpointMsg.SerializeToString fail, skip this msg";
    return -1;
  }
...
void Base :: PackBaseMsg(const std::string & sBodyBuffer, const int iCmd,
			 std::string & sBuffer, int msgType) {
  ...
  if (!bSucc) {
    PAXOSLOG_ERROR << "Header.SerializeToString fail, skip this msg";
    assert(bSucc == true);
  }
  ...
  sBuffer = string(sGroupIdx, sizeof(sGroupIdx)) + string(sHeaderLen,sizeof(sHeaderLen))
    + sHeaderBuffer + sBodyBuffer;
...
int Base :: UnPackBaseMsg(const std::string & sBuffer, Header & oHeader,
			  size_t & iBodyStartPos, size_t & iBodyLen) {
  ...
  if (iBodyStartPos > sBuffer.size()) {
    PAXOSLOG_ERROR << "Header headerlen too long " << iHeaderLen;
    return -1;
  }

  bool bSucc = oHeader.ParseFromArray(sBuffer.data() + iHeaderStartPos, iHeaderLen);
  if (!bSucc) {
    PAXOSLOG_ERROR << "Header.ParseFromArray fail, skip this msg";
    return -1;
  }

  PAXOSLOG_DEBUG << "buffer_size " << sBuffer.size() << " header len " << iHeaderLen << " cmdid "
		 << oHeader.cmdid() << " gid " << oHeader.gid() << " rid " << oHeader.rid()
		 << " version " << oHeader.version() << " body_startpos " << iBodyStartPos;
  if (oHeader.version() >= 1) {
    if (iBodyStartPos + CHECKSUM_LEN > sBuffer.size()) {
      PAXOSLOG_ERROR << "no checksum, body start pos " << iBodyStartPos << " buffersize " << sBuffer.size();
      return -1;
    }
    ...
    if (iNewCalBufferChecksum != iBufferChecksum) {
      PAXOSLOG_ERROR << "Data.bring.checksum " << iBufferChecksum << " not equal to Data.cal.checksum "
		     << iNewCalBufferChecksum;
      return -1;
    }
...
int Base :: SendMessage(const nodeid_t iSendtoNodeID, const CheckpointMsg & oCheckpointMsg,
			const int iSendType) {
  if (iSendtoNodeID == m_poConfig->GetMyNodeID()) {
    PAXOSLOG_DEBUG << "SendtoNodeID is local ip, so ignore";
    return 0; 
  }
...
int Base::BroadcastMessage(const PaxosMsg & oPaxosMsg, const int iRunType, const int iSendType) {
  ...
  if (iRunType == BroadcastMessage_Type_RunSelf_Final) {
    m_poInstance->OnReceiveMessage(&sBuffer, (const int)sBuffer.size());
  }
