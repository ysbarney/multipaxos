...
/////////////////////////////////////////////////////
void split_string_vector(const std::string& src_str, const char* comma, std::vector<std::string>& vec)
{
	boost::char_separator<char> sep(comma);
	typedef boost::tokenizer<boost::char_separator<char> > CustomTokenizer;
	CustomTokenizer tok(src_str, sep);
	for(CustomTokenizer::iterator it=tok.begin(); it!=tok.end();++it)
		vec.push_back(*it);
}
