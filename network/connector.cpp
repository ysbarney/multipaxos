...
Connector::Connector(EventLoop* l, TCPClient* client)
    ...
    PAXOSLOG_TRACE << "raddr=" << remote_addr_;
...
void Connector::Start() {
    PAXOSLOG_TRACE << "Try to connect " << remote_addr_ << " status=" << StatusToString();
    assert(loop_->IsInLoopThread());
    ...
    PAXOSLOG_TRACE << "The remote address " << remote_addr_ << " is a host, try to resolve its IP address.";
...
void Connector::Cancel() {
    PAXOSLOG_TRACE << "Cancel to connect " << remote_addr_ << " status=" << StatusToString();
    assert(loop_->IsInLoopThread());
    if (dns_resolver_) {
...
void Connector::Connect() {
    PAXOSLOG_TRACE << remote_addr_ << " status=" << StatusToString();
    ...
    if (!laddr.empty()) {
        ...
        if (rc != 0) {
            int serrno = errno;
            PAXOSLOG_ERROR << "bind failed, errno=" << serrno << " " << strerror(serrno);
            HandleError();
            return;
        }
    }
    ...
    PAXOSLOG_TRACE << "new FdChannel p=" << chan_.get() << " fd=" << chan_->fd();
...
void Connector::HandleWrite() {
    ...
    if (status_ == kDisconnected) {
        ...
        PAXOSLOG_INFO << "fd=" << chan_->fd() << " remote_addr=" << remote_addr_ << " receive write event when socket is closed";
        return;
    }
    ...
    if (getsockopt(chan_->fd(), SOL_SOCKET, SO_ERROR, (char*)&err, (socklen_t*)&len) != 0) {
        err = errno;
        PAXOSLOG_ERROR << "getsockopt failed err=" << err << " " << strerror(err);
    }
...
void Connector::HandleError() {
    ...
    PAXOSLOG_ERROR << "this=" << this << " status=" << StatusToString() << " fd=" << fd_  << " use_count=" << self.use_count() << " errno=" << serrno << " " << strerror(serrno);
    ...
    if (owner_tcp_client_->auto_reconnect()) {
        ...
        if (fd_ > 0) {
            PAXOSLOG_TRACE << "Connector::HandleError close(" << fd_ << ")";
            assert(own_fd_);
            EVUTIL_CLOSESOCKET(fd_);
            fd_ = INVALID_SOCKET;
        }

        PAXOSLOG_TRACE << "loop=" << loop_ << " auto reconnect in " << owner_tcp_client_->reconnect_interval().Seconds() << "s thread=" << std::this_thread::get_id();
...
void Connector::OnConnectTimeout() {
    PAXOSLOG_WARN << "this=" << this << " Connector::OnConnectTimeout status=" << StatusToString() << " fd=" << fd_ << " this=" << this;
...
void Connector::OnDNSResolved(const std::vector <struct in_addr>& addrs) {
    PAXOSLOG_TRACE << "addrs.size=" << addrs.size() << " this=" << this;
    if (addrs.empty()) {
        PAXOSLOG_ERROR << "this=" << this << " DNS Resolve failed. host=" << dns_resolver_->host();
        HandleError();
        return;
    }
