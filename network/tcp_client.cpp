...
TCPClient::TCPClient(EventLoop* l, const std::string& raddr, const std::string& n)
    ...
    PAXOSLOG_TRACE << "remote addr=" << raddr;
}
...
void TCPClient::Connect() {
    PAXOSLOG_INFO << "remote_addr=" << remote_addr();
    auto f = [this]() {
...
void TCPClient::DisconnectInLoop() {
    PAXOSLOG_WARN << "TCPClient::DisconnectInLoop this=" << this << " remote_addr=" << remote_addr_;
    assert(loop_->IsInLoopThread());
    auto_reconnect_.store(false);

    if (conn_) {
        PAXOSLOG_TRACE << "Close the TCPConn " << conn_.get() << " status=" << conn_->StatusToString();
    ...
    if (connector_->IsConnected() || connector_->IsDisconnected()) {
        PAXOSLOG_TRACE << "Nothing to do with connector_, Connector::status=" << connector_->status();
    } else {
...
void TCPClient::Reconnect() {
    PAXOSLOG_TRACE << "Try to reconnect to " << remote_addr_ << " in " << reconnect_interval_.Seconds() << "s again";
    Connect();
}
...
void TCPClient::OnConnection(multipaxos_socket_t sockfd, const std::string& laddr) {
    if (sockfd < 0) {
        PAXOSLOG_TRACE << "Failed to connect to " << remote_addr_ << ". errno=" << errno << " " << strerror(errno);
    ...
    PAXOSLOG_TRACE << "Successfully connected to " << remote_addr_;
    assert(loop_->IsInLoopThread());
