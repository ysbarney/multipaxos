...
class EventLoop;
class MULTIPAXOS_EXPORT EventWatcher {
public:
...
class MULTIPAXOS_EXPORT PipeEventWatcher : public EventWatcher {
public:
    ...
    bool AsyncWait();
    void Notify();
    multipaxos_socket_t wfd() const { return pipe_[0]; }
private:
    virtual bool DoInit();
    virtual void DoClose();
    static void HandlerFn(multipaxos_socket_t fd, short which, void* v);

    multipaxos_socket_t pipe_[2]; // Write to pipe_[0] , Read from pipe_[1]
};

class MULTIPAXOS_EXPORT TimerEventWatcher : public EventWatcher {
public:
...
class MULTIPAXOS_EXPORT SignalEventWatcher : public EventWatcher {
public:
