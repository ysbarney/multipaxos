...
EventLoop::EventLoop(struct event_base* base)
    ...
    assert(rc);
    if (!rc) {
        PAXOSLOG_FATAL << "PipeEventWatcher init failed.";
    }
...
void EventLoop::Run() {
    ...
    if (rc == 1) {
        PAXOSLOG_ERROR << "event_base_dispatch error: no event registered";
    } else if (rc == -1) {
        int serrno = errno;
        PAXOSLOG_ERROR << "event_base_dispatch error " << serrno << " " << strerror(serrno);
    }

    // Make sure watcher_ does construct, initialize and destruct in the same thread.
    watcher_.reset();
    PAXOSLOG_TRACE << "EventLoop stopped, tid=" << std::this_thread::get_id();
...
void EventLoop::Stop() {
    ...
    PAXOSLOG_TRACE << "EventLoop::Stop";
    QueueInLoop(std::bind(&EventLoop::StopInLoop, this));
}

void EventLoop::StopInLoop() {
    PAXOSLOG_TRACE << "EventLoop is stopping now, tid=" << std::this_thread::get_id();
    assert(status_.load() == kStopping);

    auto f = [this]() {
        for (int i = 0;;i++) {
            PAXOSLOG_TRACE << "calling DoPendingFunctors index=" << i;
            DoPendingFunctors();
    ...
    PAXOSLOG_TRACE << "before event_base_loopexit, we invoke DoPendingFunctors";

    f();

    PAXOSLOG_TRACE << "start event_base_loopexit";
    event_base_loopexit(evbase_, nullptr);
    PAXOSLOG_TRACE << "after event_base_loopexit, we invoke DoPendingFunctors";

    f();

    PAXOSLOG_TRACE << "end of StopInLoop";
}
...
void EventLoop::AfterFork() {
    ...
    if (rc != 0) {
        PAXOSLOG_FATAL << "event_reinit failed!";
        abort();
    }
...
multipaxos::InvokeTimerPtr EventLoop::RunEvery(Duration interval, const Functor& f) {
    //DLOG_TRACE;
...
multipaxos::InvokeTimerPtr EventLoop::RunEvery(Duration interval, Functor&& f) {
    //DLOG_TRACE;
...
void EventLoop::QueueInLoop(const Functor& cb) {
    PAXOSLOG_TRACE << "pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    {
#ifdef H_HAVE_BOOST
    ...
    PAXOSLOG_TRACE << "queued a new Functor. pending_functor_count_=" << pending_functor_count_ << " PendingQueueSize=" << GetPendingQueueSize() << " notified_=" << notified_.load();
    if (!notified_.load()) {
        PAXOSLOG_TRACE << "call watcher_->Nofity() notified_.store(true)";
        ...
        if (watcher_) {
            ...
            PAXOSLOG_TRACE << "status=" << StatusToString();
            assert(!IsRunning());
        }
    } else {
         PAXOSLOG_TRACE << "No need to call watcher_->Nofity()";
    }
}
...
void EventLoop::QueueInLoop(Functor&& cb) {
    ...
    if (!notified_.load()) {
        ...
        if (watcher_) {
            ...
            PAXOSLOG_TRACE << "watcher_ is empty, maybe we call EventLoop::QueueInLoop on a stopped EventLoop. status=" << StatusToString();
            assert(!IsRunning());
        }
    } else {
        PAXOSLOG_TRACE << "No need to call watcher_->Nofity()";
