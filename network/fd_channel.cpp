...
FdChannel::FdChannel(EventLoop* l, multipaxos_socket_t f, bool r, bool w)
    : loop_(l), attached_(false), event_(nullptr), fd_(f) {
    PAXOSLOG_TRACE << "fd=" << fd_;
    assert(fd_ > 0);
...
void FdChannel::AttachToLoop() {
    ...
    if (EventAdd(event_, nullptr) == 0) {
        PAXOSLOG_TRACE << "fd=" << fd_ << " watching event " << EventsToString();
        attached_ = true;
    } else {
        PAXOSLOG_ERROR << "this=" << this << " fd=" << fd_ << " with event " << EventsToString() << " attach to event loop failed";
    }
}
...
void FdChannel::DetachFromLoop() {
    ...
    if (EventDel(event_) == 0) {
        attached_ = false;
        PAXOSLOG_TRACE << "fd=" << fd_ << " detach from event loop";
    } else {
        PAXOSLOG_ERROR << "DetachFromLoop this=" << this << "fd=" << fd_ << " with event " << EventsToString() << " detach from event loop failed";
    }
}
...
void FdChannel::HandleEvent(multipaxos_socket_t sockfd, short which, void* v) {
    FdChannel* c = (FdChannel*)v;
    c->HandleEvent(sockfd, which);
}

void FdChannel::HandleEvent(multipaxos_socket_t sockfd, short which) {
    assert(sockfd == fd_);
    //PAXOSLOG_TRACE << "fd=" << sockfd << " " << EventsToString();
